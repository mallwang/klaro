...
- **Summary email** — users can opt into a weekly (every Monday) or monthly (1st of each month) contract summary email delivered at 10:00 UTC; the email lists total monthly spending, a per-contract breakdown ordered by monthly cost, highest first (equal costs fall back to alphabetical order), upcoming renewals, a dashboard link, and a context-aware call to action; preferences are managed in Account Settings, which also displays the next scheduled send datetime; anonymized contract names are hidden in the email
