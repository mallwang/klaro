...
- **Dashboard** — gesamte monatliche Ausgaben aktiver Verträge, Aufschlüsselung nach Kategorie, anstehende Verlängerungen (innerhalb von 30 Tagen) und abgelaufene aktive Verträge; deaktivierte Verträge werden in einem eingeklappten, grau dargestellten Abschnitt "Inaktive Verträge" aus dem Fokus gehalten, der nur erscheint, wenn solche Verträge vorhanden sind, und bis zum Aufklappen lediglich deren Anzahl anzeigt
...
- **Öffentliche Selbstregistrierung** — Besucher können über einen „Registrieren"-Link auf der Anmeldeseite ein Konto beantragen, auch ohne Einladung; nach Eingabe von E-Mail-Adresse und Passwort bestätigen sie ihre Adresse über einen per E-Mail zugesandten Link, wodurch die Anfrage in die Prüfwarteschlange der Administratoren verschoben wird (alle Administratoren werden per E-Mail benachrichtigt); Administratoren sehen die wartende Anfrage in einer Tabelle „Registrierungsanfragen" auf der Kontenseite und können sie genehmigen (dabei wird ein aktives Konto angelegt und die übliche Willkommens-E-Mail versendet) oder mit optionaler Begründung ablehnen (die Adresse bleibt für eine erneute Registrierung gesperrt, bis der abgelehnte Eintrag gelöscht wird)
...
- **Passwort vergessen** — Benutzer können einen Link zum Zurücksetzen des Passworts per E-Mail anfordern; der Link läuft nach 1 Stunde ab und ist nur einmal gültig; der Ablauf verhindert E-Mail-Aufzählung durch eine generische Erfolgsmeldung unabhängig von der Gültigkeit der E-Mail-Adresse; das Anmelde- und das „Passwort vergessen"-Formular teilen sich eine zweispaltige Authentifizierungsseite mit einem dekorativen Bildfenster und wechseln ohne vollständigen Seitenneuladen zwischen einander
- **Zusammenfassungs-E-Mail** — Benutzer können eine wöchentliche (jeden Montag) oder monatliche (1. des Monats) Vertragszusammenfassung per E-Mail abonnieren, die um 10:00 UTC zugestellt wird; die E-Mail enthält die monatlichen Gesamtausgaben, eine nach monatlichen Kosten absteigend sortierte Aufschlüsselung pro Vertrag (bei gleichen Kosten alphabetische Reihenfolge), bevorstehende Verlängerungen, einen Dashboard-Link und einen kontextsensitiven Handlungsaufruf; Einstellungen werden in den Kontoeinstellungen verwaltet, die auch den nächsten geplanten Sendezeitpunkt anzeigen; anonymisierte Vertragsnamen werden in der E-Mail verborgen
...
- **Admin-Diagnose** — eine nur für Administratoren zugängliche Seite unter `/admin/diagnostics` (auch über **Diagnose** im Admin-Bereich der Seitennavigation erreichbar), die Anwendungs-, Datenbank- und Laufzeitversionen, Live-Systemprüfungen (Reverse-Proxy-Erkennung, Domain-/HTTPS-Abgleich, Container-Status, ausgehender Internetzugang, DNS-Auflösung, Uhrenabweichung, WebSocket-Unterstützungsstatus) und die Konfiguration von Umgebungsvariablen anzeigt (SMTP-Host/-Port/-Absenderadresse/-Absendername im Klartext, logo.dev nur als konfiguriert/nicht konfiguriert), gruppiert in die Abschnitte „Versionen", „Systemprüfungen" und „Umgebungsvariablen"; SMTP-Zugangsdaten (`SMTP_USER`/`SMTP_PASSWORD`) werden nie angezeigt, nur die nicht-geheimen Verbindungseinstellungen; jede Live-Prüfung läuft mit einem eigenen Timeout, sodass eine langsame oder fehlgeschlagene Prüfung nie die restliche Seite blockiert; ein reiner HTML-Fallback unter demselben Pfad funktioniert auch mit deaktiviertem JavaScript
